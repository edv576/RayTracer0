...
class BoundingVolumeH : public AccelerationStructure
{
	...
	private:
		...
		void InsertInOctree(Node*& node, const Extensions* extensions, const BoundingBox<>& boundingBox, int d)
		{
			if (node->leaf) {
				if (node->nodeExtensionsList.size() == 0 || d == 8) {
		...
		void BuildOctree(Node*& node,  BoundingBox<>& boundingBox)
		{
			Vect tempCB0;
			Vect tempCB1;
			if (node->leaf) {
				...
				for (int i = 0; i < 8; i++) {
					if (node->children[i]) {
						...
						if (i & 4) {
							childBoundingBox[0].setVectX(centroid.getVectX());
						}
						else {
							childBoundingBox[0].setVectX(tempCB0.getVectX());

						}

						if (i & 4) {
							childBoundingBox[1].setVectX(tempCB1.getVectX());
						}
						else {
							childBoundingBox[1].setVectX(centroid.getVectX());
						}
...
public:
	...
	bool Intersect(const Vect&, const Vect&, double&, int&, double, int&) const;
	~BoundingVolumeH() { delete octreeH; }
	int getNIntersection();
	int nIntersections;
};
...
bool BoundingVolumeH::Extensions::intersect(
	const double* preNum,
	const double* preDen,
	double& tN,   
	double& tF,    
...
int BoundingVolumeH::getNIntersection() {

	return nIntersections;

}

bool BoundingVolumeH::Intersect(const Vect& orig, const Vect& dir, double& tH, int& index, double tLight, int& nIntersections) const 
{
	tH = infinityD;
	index = -1;
	Vect temp;
	...
	double preNum[BoundingVolumeH::nPlaneNormals];
	double preDen[BoundingVolumeH::nPlaneNormals];
	for (int i = 0; i < nPlaneNormals; i++) {
		temp = planeNormals[i];
